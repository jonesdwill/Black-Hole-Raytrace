...
function compute_physics(cfg::Config)
    ...
    Threads.@threads for i in 1:cfg.height  
        ...
        for j in 1:cfg.width
            ...
            sol = solve(prob, Tsit5(), reltol=1e-6, abstol=1e-6, callback=disk_cb, save_everystep=false)    # Solve ODE
            # !!! END ODE SOLVE !!!
            ...
            if hit_disk
                try
                    ...
                    # ARTIFACT FIX: The g < 3.0 filter was too aggressive, cutting out the brightest
                    # parts of the disk on the approaching (left) side and creating a jagged edge.
                    # We raise the limit significantly to capture these pixels.
                    if isfinite(g) && g > 0.1 && g < 10.0
                        data_mask[i, j] = true
...
function render_animation(cfg::Config)
    ...
    Threads.@threads for f in 1:frames 
        ...
        bloomed = local_frame_hdr# .+ (glow .* 0.4) # Reduced bloom to decrease blurriness
