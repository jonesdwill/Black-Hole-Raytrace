...
const CFG = Config(
    ...
    1,      # Duration 
    60,     # FPS
    joinpath(OUTPUT_DIR, "black_hole_data_480p.jld2"),
    joinpath(OUTPUT_DIR, "black_hole_480p.gif") 
)
...
function compute_physics(cfg::Config)
    ...
    Threads.@threads for i in 1:cfg.height  
        ...
        for j in 1:cfg.width
            ...
            sol = solve(prob, Tsit5(), reltol=1e-9, abstol=1e-9, callback=CallbackSet(disk_cb, horizon_cb), save_everystep=false)    # Solve ODE
            # !!! END ODE SOLVE !!!
